  ...
  options.services.sparkyfitness = {
    ...
    garmin = {
      ...
      bindAddress = lib.mkOption {
        ...
        default = "127.0.0.1";
        description = ''
          Address the Garmin microservice binds to. Keep this on the loopback
          interface: the microservice has no authentication of its own and
          accepts Garmin Connect credentials in request bodies. Only change it
          if the backend runs on a different host, and firewall it accordingly.
        '';
      };
  ...
  config = lib.mkIf cfg.enable {
    ...
    systemd.services.sparkyfitness-garmin = lib.mkIf cfg.garmin.enable {
      ...
      serviceConfig = {
        ...
        # service.py creates ./mock_data at import time, so the working
        ...
        # Left off deliberately: curl-cffi goes through libffi, which allocates
        # executable closures.
