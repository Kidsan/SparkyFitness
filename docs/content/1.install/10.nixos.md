...
::alert{type="warning"}
Keep `garmin.bindAddress` on the loopback interface. The microservice has no
authentication of its own and accepts Garmin Connect credentials in request
bodies. Only change it if the backend runs on a different host, and firewall the
port accordingly.
::
